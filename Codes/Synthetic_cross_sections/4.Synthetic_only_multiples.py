...
def process_rf_files_multiples(RF_files, grid_lon, grid_lat, grid_depth, stacking_smooth_fresnel, gauss_mult):
    ...
    for count, rf_file in enumerate(RF_files, start=1):
        ...
        if sta_id=='APAL' or sta_id=='OLGR' or sta_id== 'MYNG' or sta_id=='HELL' or sta_id== 'OSVA' or sta_id=='VITI' or sta_id == 'OSKV' or sta_id=='ATOP' or sta_id=='JONG' or sta_id=='JONS' or sta_id=='GODA' or sta_id=='VBOR' or sta_id=='KLUR':
            with open('/raid2/cg812/Synthetic_RF_with_two_chambers/Gauss_2_icelandic_crust.pkl', 'rb') as pick_file:
...
def plot_cross_section(output_path, grid_dist, grid_depth, weight_norm_vol, maskdist, maskdep, 
    ...
    axes[0].plot(29.4, 0, marker= '^', color= 'black')
    axes[0].invert_yaxis()
    ...
    axes[1].plot(29.4, 0, marker= '^', color= 'black')
    axes[1].invert_yaxis()
    ...
    plt.tight_layout()
    plt.savefig(f"{output_path}/ccp_Multiphase_cross-section_{startlat}_{startlon}_{endlat}_{endlon}_fres_{stacking_smooth_fresnel}_Gauss{gauss_mult}_EW_C_icelandic_crust.png", dpi=300, bbox_inches="tight")
    plt.close()
...
def save_outputs(
    ...
    outfile = f"{output_path}/CROSS-SECTION_Multiphase_{startlat}_{startlon}_{endlat}_{endlon}_fres_{stacking_smooth_fresnel}_Gauss_{gauss_mult}_slice_EW_C_icelandic_crust.txt"
    ...
    outfile_pos = f"{output_path}/CROSS-SECTION_Multiphase_{startlat}_{startlon}_{endlat}_{endlon}_fres_{stacking_smooth_fresnel}_Gauss_{gauss_mult}_peakspos_EW_C_icelandic_crust.txt"
    ...
    outfile_neg = f"{output_path}/CROSS-SECTION_Multiphase_{startlat}_{startlon}_{endlat}_{endlon}_fres_{stacking_smooth_fresnel}_Gauss_{gauss_mult}_peaksneg_EW_C_icelandic_crust.txt"
    ...
    outfile_mask = f"{output_path}/CROSS-SECTION_Multiphase_{startlat}_{startlon}_{endlat}_{endlon}_fres_{stacking_smooth_fresnel}_Gauss_{gauss_mult}_EW_C_icelandic_crust.txt"
    ...
    output_file = f"{output_path}/CROSS-SECTION_Multiphase_{startlat}_{startlon}_{endlat}_{endlon}_fres_{stacking_smooth_fresnel}_Gauss{gauss_direct}_icelandic_crust.pkl"
...
output_path = '/raid2/cg812/Synthetic_RF_with_two_chambers'        # Path to save the results (output png and text files).    # Path to RF data that will be processed for PS.       # Path to RF data that will be processed for multiples.
...
RF_files_mult = ['/raid2/cg812/Synthetic_RF_with_two_chambers/Gauss_2_icelandic_crust.pkl']*len(glob.glob('/raid2/cg812/Migrated_to_depth/2/*'))
