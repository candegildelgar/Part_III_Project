...
def normalize_and_mask(vol, vol_weight, vol_num, SD_vol_running, grid_lat, grid_depth, roll_av, mask_out_val=0.1):
    ...
    mask = (vol_sign[..., 0] == vol_sign[..., 1]) & (vol_sign[..., 0] == -vol_sign[..., 2]) & (vol_weight[...,0] > mask_out_val) &(vol_weight[...,1] > mask_out_val)
...
def plot_cross_section(output_path, grid_dist, grid_depth, weight_norm_vol, maskdist, maskdep, 
    ...
    plt.tight_layout()
    plt.savefig(f"{output_path}/ccp_Multiphase_cross-section_{startlat}_{startlon}_{endlat}_{endlon}_fres_{stacking_smooth_fresnel}_Gauss{gauss_direct}_{gauss_mult}_Vs_F.png", dpi=300, bbox_inches="tight")
    plt.close()
...
def save_outputs(
    ...
    outfile = f"{output_path}/CROSS-SECTION_Multiphase_{startlat}_{startlon}_{endlat}_{endlon}_fres_{stacking_smooth_fresnel}_Gauss{gauss_direct}_{gauss_mult}_slice_Vs_F.txt"
    ...
    outfile_pos = f"{output_path}/CROSS-SECTION_Multiphase_{startlat}_{startlon}_{endlat}_{endlon}_fres_{stacking_smooth_fresnel}_Gauss{gauss_direct}_{gauss_mult}_peakspos_Vs_F.txt"
    ...
    outfile_neg = f"{output_path}/CROSS-SECTION_Multiphase_{startlat}_{startlon}_{endlat}_{endlon}_fres_{stacking_smooth_fresnel}_Gauss{gauss_direct}_{gauss_mult}_peaksneg_Vs_F.txt"
    ...
    outfile_mask = f"{output_path}/CROSS-SECTION_Multiphase_{startlat}_{startlon}_{endlat}_{endlon}_fres_{stacking_smooth_fresnel}_Gauss{gauss_direct}_{gauss_mult}_mask_Vs_F.txt"
    ...
    output_file = f"{output_path}/CROSS-SECTION_Multiphase_{startlat}_{startlon}_{endlat}_{endlon}_fres_{stacking_smooth_fresnel}_Gauss{gauss_direct}_{gauss_mult}_dict_Vs_F.pkl"
...
output_path = '/raid2/cg812/Cross_sections_N_S_for_3D'        # Path to save the results (output png and text files).
RF_folder_direct = f'/raid2/cg812/Migrated_to_depth/6'    # Path to RF data that will be processed for PS.
RF_folder_mult = f'/raid2/cg812/Migrated_to_depth/2'        # Path to RF data that will be processed for multiples.
mask_out_val = 0.01                                                 # Masking threshold.
startlat, startlon, endlat, endlon = 65.025, -16.14, 65.025, -17.14    # Start and end points of the cross-section.
...
grid_lon, grid_lat, grid_depth, grid_dist = load_grid(f'/raid2/cg812/Grids/cross-section_NS_F.lonlat', 60)

RF_files_direct = glob.glob(f"{RF_folder_direct}/*.pkl")
RF_files_mult = glob.glob(f"{RF_folder_mult}/*.pkl")


#RF_files_mult= glob.glob('/raid2/cg812/Migrated_to_depth_only_inside_caldera/2/*')
