...
def load_grid(grid_file, depth_max=125.0, depth_step=0.1):
    ...
    grid_dist = np.arange(len(grid_lat)) * 1    # Assuming 5 km spacing between grid points (as used in script 1b)

    return grid_lon, grid_lat, grid_depth, grid_dist



def process_rf_files_multiples(RF_files, grid_lon, grid_lat, grid_depth, stacking_smooth_fresnel, gauss_mult):
    """
    ...
    """
    shape = (len(grid_lon), len(grid_depth), 3)
    vol = np.zeros(shape)
    vol_weight = np.zeros(shape)
    vol_num = np.zeros(shape)
    SD_vol_running = np.zeros((len(grid_lon), len(grid_depth), 3))
    Mn_running = np.zeros((len(grid_lon), len(grid_depth), 2, 3))
    Sn_running = np.zeros((len(grid_lon), len(grid_depth), 2, 3))
    Wn_running = np.zeros((len(grid_lon), len(grid_depth), 2, 3))
    ...
    for count, rf_file in enumerate(RF_files, start=1):
        ...
        stat_lat, stat_lon = float(data[0].stats['stla']), float(data[0].stats['stlo'])
        BAZ = float(data[0].stats['baz'])
        ...
        for la in valid_idx:
            ...
            Vs = 6.4 / 1.78
            FZHW_Vs = fresnel_half_width(grid_depth[:n_pps], Vs, 11)
            ...
            for d in range(n_pps):
                ...
                w = weight_PPs[d]
                vol[la, d, 0] += w * current_RF_amp_PPs
                vol_weight[la, d, 0] += w
                if w > 0:
                    vol_num[la, d, 0] += 1
            ...
            Vs = 6.4 / 1.78
            FZHW_Vs = fresnel_half_width(grid_depth[:n_pss], Vs, 11)
            ...
            for d in range(n_pss):
                ...
                w = weight_PSs[d]
                vol[la, d, 1] += w * current_RF_amp_PSs
                vol_weight[la, d, 1] += w
                if w > 0:
                    vol_num[la, d, 1] += 1
...
def normalize_and_mask(vol, vol_weight, vol_num, SD_vol_running, grid_lat, grid_depth, roll_av, mask_out_val=0.1):
    ...
    mask = (vol_sign[..., 0] == -vol_sign[..., 1]) &  (vol_weight[...,0] > mask_out_val) & (vol_weight[...,1] > mask_out_val) 
    weight_norm_vol[..., 2] = np.where(mask, weight_norm_vol[..., 0] - weight_norm_vol[..., 1], weight_norm_vol[..., 2])
    #weight_norm_vol[..., 2] = weight_norm_vol[..., 0] - weight_norm_vol[..., 1]

    # Smooth merged grid
    vol_merge = weight_norm_vol[:, :, 2]
    vol_merge_SD = np.sqrt(np.sum(SD_vol_running[:, :, :2] ** 2, axis=2))

    # Initialize arrays for plotting
    vol_plot_merge = np.zeros((len(grid_lat), len(grid_depth)))
    vol_plot_merge_SD = np.zeros((len(grid_lat), len(grid_depth)))
    #vol_plot_merge= vol_merge
    #vol_plot_merge_SD= vol_merge_SD
...
def pick_peaks(weight_norm_vol, vol_weight, grid_lat, grid_lon, grid_dist, grid_depth, vol_plot_merge, vol_merge_SD_minus2SE, vol_merge_SD_plus2SE, SD_vol_minus2SD_running, SD_vol_plus2SD_running, roll_av=50, min_amp=0.04, mask_out_val=0.1):
    ...
    for i in range(len(grid_lat)):
        ...
        for dep_index in range(roll_av, (len(grid_depth) - 1) - roll_av):
            ...
            prev_amp_PPS = sum(weight_norm_vol[[i]*len(roll_array),dep_index+roll_array_prev,0])/roll_av
            cur_amp_PPS = sum(weight_norm_vol[[i]*len(roll_array),dep_index+roll_array,0])/roll_av
            next_amp_PPS = sum(weight_norm_vol[[i]*len(roll_array),dep_index+roll_array_next,0])/roll_av
            SD_cur_PPS = SD_vol_minus2SD_running[i,dep_index,0]
            SD_cur_plus_PPS = SD_vol_plus2SD_running[i,dep_index,0]
            true_amp_PPS = weight_norm_vol[i,dep_index,0]

            prev_amp_PSS = sum(weight_norm_vol[[i]*len(roll_array),dep_index+roll_array_prev,1])/roll_av
            cur_amp_PSS = sum(weight_norm_vol[[i]*len(roll_array),dep_index+roll_array,1])/roll_av
            next_amp_PSS = sum(weight_norm_vol[[i]*len(roll_array),dep_index+roll_array_next,1])/roll_av
            SD_cur_PSS = SD_vol_minus2SD_running[i,dep_index,1]
            SD_cur_plus_PSS = SD_vol_plus2SD_running[i,dep_index,1]
            true_amp_PSS = weight_norm_vol[i,dep_index,1]
...
def plot_cross_section(output_path, grid_dist, grid_depth, weight_norm_vol, maskdist, maskdep, 
    ...
    axes[0].plot(dist_peaks_PPS, dep_peaks_PPS, 'ro')
    axes[0].plot(dist_peaks_neg_PPS, dep_peaks_neg_PPS, 'bo')
    axes[0].invert_yaxis()
    axes[0].set_ylabel("Depth (km)")
    axes[0].set_title("PPS phase")

    # PSS phase
    pc2 = axes[1].pcolormesh(grid_dist, grid_depth, weight_norm_vol[:,:,1].T, vmin=-color_saturate, vmax=color_saturate, cmap='RdYlBu')
    axes[1].plot(maskdist, maskdep, 'ws', markeredgewidth=0.0)
    axes[1].plot(dist_peaks_PSS, dep_peaks_PSS, 'ro')
    axes[1].plot(dist_peaks_neg_PSS, dep_peaks_neg_PSS, 'bo')
    axes[1].invert_yaxis()
    axes[1].set_ylabel("Depth (km)")
    axes[1].set_title("PSS phase")

    # Merged phase
    pc3 = axes[2].pcolormesh(grid_dist, grid_depth, vol_plot_merge.T, vmin=-color_saturate, vmax=color_saturate, cmap='RdYlBu_r')
    axes[2].plot(maskdist, maskdep, 'ws', markeredgewidth=0.0)
    axes[2].plot(dist_peaks_merge_max, dep_peaks_merge_max, 'ro')
    axes[2].plot(dist_peaks_neg_merge, dep_peaks_neg_merge, 'bo')
    axes[2].set_xlabel("Distance (km)")
    axes[2].set_ylabel("Depth (km)")
    axes[2].invert_yaxis()
    axes[2].set_title("Multiphase Stack")

    plt.tight_layout()
    plt.savefig(f"{output_path}/ccp_Multiphase_cross-section_{startlat}_{startlon}_{endlat}_{endlon}_fres_{stacking_smooth_fresnel}_Gauss_{gauss_mult}_EW_C_no_smoothing_whole_crust.png", dpi=300, bbox_inches="tight")
    plt.close()
...
def save_outputs(
    ...
    weight_norm_vol[:,:,2]=vol_plot_merge
    SD_vol_minus2SD_running[:,:,2] = vol_merge_SD_minus2SE
    SD_vol_plus2SD_running[:,:,2] = vol_merge_SD_plus2SE

    # Save cross-section to a text file
    outfile = f"{output_path}/CROSS-SECTION_Multiphase_{startlat}_{startlon}_{endlat}_{endlon}_fres_{stacking_smooth_fresnel}_Gauss_{gauss_mult}_slice_EW_C_no_smoothing_whole_crust.txt"
    ...
    outfile_pos = f"{output_path}/CROSS-SECTION_Multiphase_{startlat}_{startlon}_{endlat}_{endlon}_fres_{stacking_smooth_fresnel}_Gauss_{gauss_mult}_peakspos_EW_C_no_smoothing_whole_crust.txt"
    ...
    outfile_neg = f"{output_path}/CROSS-SECTION_Multiphase_{startlat}_{startlon}_{endlat}_{endlon}_fres_{stacking_smooth_fresnel}_Gauss_{gauss_mult}_peaksneg_EW_C_no_smoothing_whole_crust.txt"
    ...
    outfile_mask = f"{output_path}/CROSS-SECTION_Multiphase_{startlat}_{startlon}_{endlat}_{endlon}_fres_{stacking_smooth_fresnel}_Gauss_{gauss_mult}_mask_EW_C_no_smoothing_whole_crust.txt"
    ...
    final_output = np.zeros([len(grid_lat), len(grid_depth), 3, 9])
    ...
    output_file = f"{output_path}/CROSS-SECTION_Multiphase_{startlat}_{startlon}_{endlat}_{endlon}_fres_{stacking_smooth_fresnel}_Gauss{gauss_mult}_dict_EW_C_no_smoothing_whole_crust.pkl"
...
gauss_direct = '2'
gauss_mult = '2'
stacking_smooth_fresnel = 0.1
output_path = '/raid2/cg812/Only_multiples'        # Path to save the results (output png and text files).    # Path to RF data that will be processed for PS.
RF_folder_mult = '/raid2/cg812/Migrated_to_depth_3D/2'        # Path to RF data that will be processed for multiples.
mask_out_val = 0.01                                                 # Masking threshold.
startlat, startlon, endlat, endlon = 65.025, -17.14, 65.025, -16.14   # Start and end points of the cross-section.
roll_av = 20                                                        # Rolling average for smoothing and peak detection. Is 2km onwards 
...
RF_files_mult = glob.glob(f"{RF_folder_mult}/*.pkl")

# Call function for processing the RF files for the PS phase.


vol, vol_weight, vol_num, SD_vol_running = process_rf_files_multiples(RF_files_mult, grid_lon, grid_lat, grid_depth, stacking_smooth_fresnel, gauss_mult)
